...
def index(request):
    return render(request, 'catalogueManagement/index.html')
...
def saveCleaningRules(request):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('save_type') == 'edit':
            data = Catalogue.objects.get(typeid = id)
            data.cataloguename = cataloguename
            data.typeparentid = typeparentid
        elif request.POST.get('save_type') == 'add':
            if Catalogue.objects.filter(typeparentid=id):
                numdata = Catalogue.objects.filter(typeparentid=id)
                listid = []
                for num in range(len(numdata)):
                    listid.append(numdata[num].typeid)
                numid = max(listid) + 1
                data = Catalogue(cataloguename = cataloguename,  typeparentid = id,typeid = numid, count = 0,  typetime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
            else:
                numid = int(id)*1000
                numid2 = numid+1
                data = Catalogue(cataloguename = cataloguename,count = 0,  typeparentid = id,  typeid = numid2, typetime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
def delCleaningRules(request):
    ...
    if request.method == 'POST':
        ...
        # print(ids)
        try:
            Catalogue.objects.extra(where=['typeid in ('+ ids + ')']).delete()
        except:
            result.errorCode = '0x0001'
            result.errorString = '数据库操作失败'
    else:
...
def allData(request):
    ...
    if len(b)>2:
        ...
        print(82910)
        data = Catalogue.objects.filter(typeid=911)
        print(data)
        print("hello")
        lens = len(data)
        list = []
